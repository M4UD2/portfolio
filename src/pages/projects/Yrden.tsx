...
export default function Yrden() {
  ...
  return (
    <ProjectDetail
      ...
      description="Yrden é uma plataforma de BI Headless (preparação e integração de dados) fictícia para empresas SaaS (B2B). Como Product Designer, liderei a estratégia de produto e descoberta para a jornada do novo usuário, definindo problemas de usuário e negócio, bem como as métricas de sucesso (KPIs). O desafio: uma limitação técnica de 60 minutos de sincronização causava uma péssima primeira impressão, impactando a Taxa de Ativação de novas contas e aumentando o Time to Value (TTV) do produto."
      ...
      solution="Em vez de uma solução de desenvolvimento de alto custo, priorizei um redesenho da experiência. Criei um fluxo de onboarding produtivo que aproveita o tempo de espera para educar o usuário e provar valor imediatamente. A solução foi dividida em: 1) Modal de Boas-vindas (contexto) - explica o que é o Yrden e por que há espera; 2) Checklist Produtivo (ação) - guia tarefas durante a espera; 3) Estratégia 'Recompensa > Esforço' - mostrar valor (relatórios demo) antes de pedir esforço (conectar dados). O resultado: transformar 60 minutos de frustração em uma jornada produtiva que reduz o Time to Value para < 5 minutos."
